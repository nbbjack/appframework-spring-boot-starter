...
import com.framework.authority.config.CloseAuthorityEvironment;
import com.framework.authority.config.CustomDaoAuthenticationProvider;
import com.framework.authority.filter.CustomAuthenticationFilter;
import com.framework.authority.handler.GlobalAccessDeniedHandler;
import com.framework.authority.handler.GlobalAuthenticationEntryPoint;
import com.framework.authority.handler.GlobalAuthenticationFailureHandler;
import com.framework.authority.handler.GlobalAuthenticationSuccessHandler;
import com.framework.authority.mobile.SmsAuthenticationConfig;
import com.framework.authority.mobile.SmsCodeAuthenticationFilter;
...
public class Auth2ResourceServerConfig extends ResourceServerConfigurerAdapter {
    ...
    private GlobalAuthenticationFailureHandler globalAuthenticationFailureHandler =
        new GlobalAuthenticationFailureHandler();
    ...
    public void configure(HttpSecurity http) throws Exception {
        ...
        if (closeAuthority == null || closeAuthority.getCloseAuthEnvironment() == null
            || closeAuthority.getCurrentRunEnvironment() == null) {
            isCloseAuth = false;
        } else {
        ...
        // 配置短信验证码过滤器
        http.addFilterBefore(new SmsCodeAuthenticationFilter(abstractCheckSmsCode, globalAuthenticationFailureHandler),
            UsernamePasswordAuthenticationFilter.class);

        // 表单登录登录配置
        http.formLogin().loginProcessingUrl(SecurityConstants.DEFAULT_LOGIN_URL_USERNAME_PASSWORD)
            .successHandler(new GlobalAuthenticationSuccessHandler(authorizationServerTokenServices,
                clientDetailsService, bCryptPasswordEncoder))
        ...
        http.addFilter(customAuthenticationFilter()).apply(new SmsAuthenticationConfig(userDetailsService,
            authorizationServerTokenServices, clientDetailsService, bCryptPasswordEncoder));

        // 访问异常以及权限异常处理器配置
        http.exceptionHandling().accessDeniedHandler(globalAccessDeniedHandler)
    ...
    CustomAuthenticationFilter customAuthenticationFilter() throws Exception {
        ...
        filter.setAuthenticationSuccessHandler(new GlobalAuthenticationSuccessHandler(authorizationServerTokenServices,
            clientDetailsService, bCryptPasswordEncoder));
