...
public class SmsAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        SmsAuthenticationToken authenticationResult =
            new SmsAuthenticationToken(userDetails.getAuthorities(), userDetails);
        authenticationResult.setDetails(smsAuthenticationToken.getDetails());
        return authenticationResult;
    }

    /**
     * AuthencationManager负责协调由那个Provider来处理对应的Filter, 具体协调的过程就是通过该方法来协调的. 判断传入的参数 是否和 PhoneNumAuthenticationToken.class
     * 为同一个类型。如果是同一个类型，就执行authenticate方法。
     *
