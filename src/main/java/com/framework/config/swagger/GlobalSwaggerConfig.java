...
public class GlobalSwaggerConfig implements WebMvcConfigurer {
    ...
    private void initConfigureDockets(List<SwaggerApiInfo> swaggerApiInfoList) {
        for (SwaggerApiInfo swaggerApiInfo : swaggerApiInfoList) {
            ...
            ApiInfo apiInfo = (new ApiInfoBuilder()).title(swaggerApiInfo.getGroupName()).description("")
                .termsOfServiceUrl("").version(swaggerApiInfo.getVersion()).build();

            Docket docket = (new Docket(DocumentationType.SWAGGER_2)).groupName(swaggerApiInfo.getGroupName())
                .enable(swaggerEnable()).apiInfo(apiInfo).select()
                .apis(RequestHandlerSelectors.basePackage(swaggerApiInfo.getBasePackage())).paths(PathSelectors.any())
                .build();
